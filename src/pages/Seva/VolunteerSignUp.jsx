...
//       base64: uploadedFileMeta?.extractedFileName ? `uploads/others/${uploadedFileMeta.extractedFileName}` : "",
...
/**
 * @file src/pages/Seva/VolunteerSignUp.jsx
 * @description 100% Dumb/Presentational Component.
 * Contains ZERO business logic, state definitions, or API handling.
 * Strictly consumes state and handlers from the `useVolunteerForm` controller hook.
 */

import { useState, useRef, useEffect } from "react";
import { APP_COLORS } from "../../constants/appColors";
import dividerImg from "../../assets/border.png";
import { useVolunteerForm } from "../../hooks/queries/Seva/VolunteerSignup/useVolunteerForm";

// --- Reusable Pure UI Primitives ---
const baseFieldClasses =
  "w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 text-[15px] font-medium text-gray-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#900000]/30 transition-all placeholder-gray-400 disabled:opacity-50 disabled:cursor-not-allowed";
...
export const VolunteerSignUp = () => {
  ...
  // 1. Hook encapsulates ALL business logic, state, and API handling
  const {
    state: {
      formData,
      isUploading,
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-3">
                {[
                  ...
                  return (
                    <label
                      key={area}
                      className={`flex items-center gap-3 cursor-pointer p-3.5 rounded-xl border transition-all duration-200 ${isSelected ? "bg-red-50/40 border-red-200 shadow-sm" : "bg-gray-50 border-transparent hover:bg-gray-100"}`}
                    >
                      <div
                        className={`relative w-5 h-5 flex-shrink-0 rounded-[4px] border flex items-center justify-center transition-colors ${isSelected ? "border-transparent" : "bg-white border-gray-300"}`}
                        style={{
                          backgroundColor: isSelected
                            ? primaryColor
                            : undefined,
                      ...
                      />
                      <span
                        className={`text-[12px] md:text-[13px] font-semibold text-left leading-tight flex-1 ${isSelected ? "" : "text-gray-700"}`}
